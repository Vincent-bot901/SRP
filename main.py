...
#For at forbedre vores minimax kan vi isteden for at få at kigge på alle mulig træk, kun kigge på de træk som er 'godt', altså det vil sige at vi kigger på mindre træk og dermed går det hurtiger.

# vores forbedred minimax funktion kalder vi alphabeta 	


def aplhaBeta(board, depth,alpha, beta, maximize):
    if(board.is_checkmate()):
        if(board.turn == chess.WHITE):
            return -100000
        else:
...
def FastMove(depth, board, maximize):
